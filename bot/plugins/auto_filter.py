...
async def auto_filter(bot, update):
    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            button_text = f"ℹ️{file_size}" button_text = f"📂{file_name}"
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("⚜️Goto Next Page⚜️", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"⚜️Check Link at @CC_links_SK ⚜️..Found {(len_results)} Results For Your Request 🥰: <code>{query}</code>",
